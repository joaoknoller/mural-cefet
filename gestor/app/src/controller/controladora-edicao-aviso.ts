...
export default class ControladoraEdicaoAviso extends ControladoraFormulario {
	...
	async executar(id: number): Promise<void> {
		try {
			...
			this.visao.preencherValidade(aviso.validade.date);
			...
			this.visao.aoEnviar(this.editarAviso, setores, periodosExibicao, publicosAlvo, id);
		} catch (error) {
			this.visao.exibirErros((error as Error).message);
		}
	}

	editarAviso = async (aviso: Aviso) => {
		try {
			await this.servico.alterar(aviso);
			this.visao.exibirSucesso(MENSAGENS.sucesso.aviso.editadoComSucesso);
		} catch (error) {
			this.visao.exibirErros((error as Error).message);
		}
	};
